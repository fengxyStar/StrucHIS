...
### This code is from https://github.com/THUDM/HGB/tree/master
...
class DataLoader:
    def __init__(self, path):
        ...
        if os.path.exists(path):
            ...
            raise NameError(f"{path} doesn't exist! Please download the dataset first.")
#         import ipdb
#         ipdb.set_trace()
    ...
    def load_links(self):
        ...
        for r_id in links['data']:
            links['data'][r_id] = torch.LongTensor(np.array(links['data'][r_id]))
#         new_data = {}
#         for r_id in links['data']:
    ...
    def load_nodes(self):
        ...
        for i in range(len(nodes['count'])):
            ...
            if shift in nodes['attr']:
                ...
                attr[i] = torch.FloatTensor(np.array(mat))
            else:
                attr[i] = None
            shift += nodes['count'][i]
        nodes['attr'] = attr
